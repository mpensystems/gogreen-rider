...
import {selectImages, selectRider, selectRiderDetails, selectToken, setImage, setRirderDetails} from 'redux/slices/riderSlice';
...
const RiderKycForm = () => {
  ...
  useEffect(() => {
    if ((!images?.utility_bill && riderDetails?.utility_bill ) || (!images?.id_front && riderDetails?.photo_id_front) || (!images?.id_back && riderDetails?.photo_id_back) || (!images?.user_photo && riderDetails?.photo) || (!images?.pan && riderDetails?.pan_copy) && st) {
      const imageIds = [
